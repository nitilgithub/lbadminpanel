...
<div class="widget-box">
  <div class="widget-title"> <span class="icon"><i class="icon-th"></i></span>
    <h5><?= !empty($pageheading) ? $pageheading : '' ?></h5>
      <?php
            ...
            if(isset($extrabtn))
            {
                foreach ($extrabtn as $ebtn)
                {
                 ...
                 <?php
                    ...
                    {
                        $modId = $ebtn['title']."myModal".dec($id);
                        if($ebtn['title'] == "Feedback")
                        {
                            ?>
                            <a style="margin-bottom: 10px"  href="#feedbackModel" data-name="<?= $uname ?>"  data-id="<?= $id ?>"  data-toggle="modal" class="btn <?= $ebtn['class'] ?> tip feedbackModel" data-original-title="View Feedback" >View Feedback</a>
                            <a  href="#addfeedbackModel" data-name="<?= $uname ?>" data-id="<?= $id ?>"  data-toggle="modal" class="btn <?= $ebtn['class'] ?> tip addfeedbackModel" data-original-title="Add Feedback" >Add Feedback</a>
                            <?php
                        }
                        elseif($ebtn['title'] == "Remarks")
                        {
                            ?>
                            <a style="margin-bottom: 10px"  href="#remarksModel" data-name="<?= $uname ?>" data-id="<?= $id ?>"  data-toggle="modal" class="btn <?= $ebtn['class'] ?> tip remarksModel" data-original-title="View Feedback" >View Remarks</a>
                            <a  href="#addremarksModel" data-name="<?= $uname ?>"  data-id="<?= $id ?>"  data-toggle="modal" class="btn <?= $ebtn['class'] ?> tip addremarksModel" data-original-title="Add Feedback" >Add Remarks</a>
                            <?php
                        }
                        else{
                  ?>
                        <a  href="#<?= $modId ?>"  data-toggle="modal" class="btn <?= $ebtn['class'] ?> tip" data-original-title="<?= $ebtn['title'] ?>" ><i class="<?= $ebtn['icon-class'] ?>"></i> <?= $ebtn['name'] ?></a>
                   <?php
                        $modelData['modId'] = $modId;
                        $modelData['id'] = $id;
                        $this->load->view('./'.$ebtn['target'],$modelData);
                        }
                    }
                    ?>
                    <?php
