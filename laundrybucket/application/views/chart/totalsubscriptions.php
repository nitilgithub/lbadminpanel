...
<div id="content">
    <div id="content-header">
        ...
        <h1><?= !empty($pageheading) ? $pageheading : '' ?> <span id="year-span" ></span></h1>
    </div>
...
<script>
    ...
    function getGraphData(year) {
        ...
        $('#year-span').html('');
        var year = $('#year').val();
        $('#year-span').append(' In '+year);

        $.ajax({
            type: 'GET',
            url: '<?php echo base_url().midurl().$mbase ?>getTotalSubscriptionsData?year='+year,
            dataType: "json",
            beforeSend: function () {
                $('#loading-wrp').show();
            },
